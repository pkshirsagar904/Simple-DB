...
public class StringAggregator implements Aggregator {
	...
	class StringAggregatorImplWithGrouping extends StringAggregatorImpl {
		...
		//	 field2aggFtn.put(new Tuple(td).getField(afield), new CountAggregateFunction());	
		...
		public void merge(Tuple tup) {
			...
			{
				AggregateFunction funct = field2aggFtn.get(tup.getField(gbfield));
				funct.merge(tup.getField(afield));
			}
			else
			{
				AggregateFunction funct2 = aggFtnFactory.createAggregateFunction();
				funct2.merge(tup.getField(gbfield));
				field2aggFtn.put(tup.getField(gbfield), funct2);
			}
		...
		public DbIterator iterator() {
			...
			while (newiterator.hasNext()) {
				...
				newtuple.setField(afield,temp.getValue().aggregateValue()); 
